"""Run product image mismatch pipeline with evaluation.

Determines whether the candidate image shows a clearly different product
from the reference image. Used as Stage 2 after items are classified as
Not_Match in Stage 1.
...
Usage:
    .venv/bin/python scripts/pipeline/run_binary_mismatch_pipeline_with_eval.py \
        ...
        --output_file_path benchmark_runs/smartwatch_mismatch_1.json \
        --category smartwatch
"""
...
def process_item(
    ...
    return {
        ...
        "score": eval_result.get("mismatch_score", 0.0),
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run product image mismatch pipeline with evaluation (Binary Mismatch vs Not_Mismatch)")
    ...
    parser.add_argument("--data_dir", required=True, help="Base data directory where images and mapping CSV reside")
    ...
    if valid_evals > 0:
        ...
        logging.info(f"[SUMMARY] Average Mismatch Score: {avg_score:.2f}")
