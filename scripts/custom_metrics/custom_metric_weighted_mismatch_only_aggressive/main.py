...
# Aggressive (~90% precision target) | TP=+1, FP=-9, FN=-0.5, TN=0 | FP:FN ratio 18:1
WEIGHTS = {"tp": 1, "fp": -9, "fn": -0.5, "tn": 0}
...
def product_mismatch_weighted_metric(request):
    try:
        ...
        logger.info(f"aggressive | Score: {score} | GT: {ground_truth}")
