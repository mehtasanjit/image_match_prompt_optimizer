...
eval_cf_names × num_iterations for binary Mismatch classification,
then evaluating each final optimized prompt on held-out splits.

Usage example (run from repo root):
    ./venv/bin/python scripts/mlflow_gepa/run_gepa_binary_mismatch_grid.py \
        --project my-gcp-project \
        --eval_cf_names img_mismatch_weighted_guarded,img_mismatch_weighted_balanced \
        --eval_score_key mismatch_score \
        ...
        --initial_prompt ./prompts/binary_mismatch_or_not.txt \
        --target_model gemini-3-flash-preview \
        --location global \
        --eval_cf_location us-central1 \
        --eval_workers 10 \
        --output_file grid_runs/mismatch/smartwatch/results.json
"""
...
# Import eval pipeline components (binary mismatch variant).
# These come from scripts/pipeline/ — a sibling directory, not part of mlflow_gepa.
try:
    from run_binary_mismatch_pipeline_with_eval import process_item, csv_to_eval_data
    from summarize_binary_mismatch_pipeline_eval import summarize_eval_results
except ImportError as e:
    raise ImportError(
        "Grid runner requires the pipeline scripts (scripts/pipeline/) to be available. "
        "Ensure run_binary_mismatch_pipeline_with_eval.py and "
        "summarize_binary_mismatch_pipeline_eval.py are in scripts/pipeline/."
    ) from e
...
def main():
    parser = argparse.ArgumentParser(
        ...
        "for binary Mismatch classification, then evaluate each final prompt on held-out splits.",
    )
    ...
    # Prompt
    parser.add_argument("--prompt_name", default="image_mismatch_system_prompt", help="MLflow prompt registry name")
    parser.add_argument(
        "--initial_prompt",
        default="prompts/binary_mismatch_or_not.txt",
    ...
    # Output
    parser.add_argument("--output_file", default="grid_results_mismatch.json", help="Path to output JSON")
    parser.add_argument("--experiment_name", default="IMAGE_MISMATCH_GEPA_GRID", help="MLflow experiment name")
    ...
    parser.add_argument("--full_data_dir", default=None,
                        help="Directory with full dataset for evaluating final prompt. Images + mapping CSV.")
    ...
    # Compute max_train_samples from subsample_fraction
...
def _write_output(path: str, results: list, grid: list):
    # Find best cell by eval set mismatch precision
    best_by_precision = None
    best_precision_val = -1.0
    # Find best cell by eval set mismatch F1
    best_by_f1 = None
    best_f1_val = -1.0
    ...
    for r in results:
        ...
        mismatch_precision = agg.get("metrics_mismatch", {}).get("precision", -1.0)
        mismatch_recall = agg.get("metrics_mismatch", {}).get("recall")
        mismatch_f1 = agg.get("metrics_mismatch", {}).get("f1", -1.0)

        cell_summary = {
            "grid_cell": r["grid_cell"],
            "eval_mismatch_precision": mismatch_precision,
            "eval_mismatch_recall": mismatch_recall,
            "eval_mismatch_f1": mismatch_f1,
            "gepa_final_eval_score": r.get("gepa", {}).get("final_eval_score"),
        }

        if mismatch_precision > best_precision_val:
            best_precision_val = mismatch_precision
            best_by_precision = cell_summary.copy()

        if mismatch_f1 > best_f1_val:
            best_f1_val = mismatch_f1
